...
def accuracy(output, target, topk=(1,)):
    ...
    with torch.no_grad():
        ...
        _, pred = output.topk(maxk, 1, True, True)  # [N, k]
        pred = pred.t()                             # [k, N]
        ...
        return res  # 返回 top1, top5 acc


def extract_positive_queries_and_labels(hs, targets, indices):
    """
    从 Transformer decoder 输出中提取前景样本（被匹配上的 query）
    - hs: [B, Q, D]
    - targets: list of target dicts
    - indices: list of (idx_pred, idx_gt) from matcher
    """
    features = []
    labels = []
    for i, (idx_pred, idx_gt) in enumerate(indices):
        if len(idx_pred) == 0:
            continue
        feats = hs[i][idx_pred]  # shape: [M, D]
        lbls = targets[i]["labels"][idx_gt]
        features.append(feats)
        labels.append(lbls)
    return torch.cat(features, dim=0), torch.cat(labels, dim=0)


def train_one_epoch_incremental(model: torch.nn.Module,
    ...
    for _ in metric_logger.log_every(range(len(data_loader)), print_freq, header):
        ...
        if getattr(model, "use_acil", False):
            ...
            if decoder_output is not None:
                with torch.no_grad():
                    ...
                    if X_fg.numel() > 0:
                        acil_features.append(X_fg)
                        acil_labels.append(y_fg)

        # 正常计算 loss
        loss_dict = criterion(outputs, targets)
        weight_dict = criterion.weight_dict

        # 屏蔽 loss_ce 梯度（ACIL 分类器不训练）
        if getattr(model, "use_acil", False) and "loss_ce" in loss_dict:
            loss_dict["loss_ce"] = loss_dict["loss_ce"].detach() * 0.0
        ...
        if getattr(model, "use_acil", False):
            with torch.no_grad():
                ...
                if decoder_output is not None:
                    ...
                    X_fg, y_fg = extract_positive_queries_and_labels(decoder_output, targets, indices)
                    if X_fg.numel() > 0:
                        logits = model.acil_classifier(X_fg)  # [N, C]
                        acc = accuracy(logits, y_fg)[0]
                        class_err = 100.0 - acc
                        metric_logger.update(class_error=class_err)
        else:
    ...
    # 🧠 一个 epoch 结束后再做 ACIL 分类器的拟合
    if getattr(model, "use_acil", False) and acil_features:
        X_all = torch.cat(acil_features, dim=0)
        y_all = torch.cat(acil_labels, dim=0)
        model.acil_classifier.fit(X_all, y_all)
        print(f"[ACIL] Fitted classifier on {X_all.shape[0]} positive query features.")
