...
class SetCriterion(nn.Module):
    ...
    def loss_labels(self, outputs, targets, indices, num_boxes,enable_aux, log=True):
    ...
    @torch.no_grad()
    def loss_cardinality(self, outputs, targets, indices, num_boxes,enable_aux):
    ...
    def loss_boxes(self, outputs, targets, indices, num_boxes,enable_aux):
    ...
    def loss_masks(self, outputs, targets, indices, num_boxes,enable_aux):
    ...
    def get_loss(self, loss, outputs, targets, indices, num_boxes,enable_aux, **kwargs):
        loss_map = {
        ...
        return loss_map[loss](outputs, targets, indices, num_boxes,enable_aux, **kwargs)
    ...
    def forward(self, outputs, targets, enable_aux=True):
        ...
        for loss in self.losses:
            kwargs = {}
            losses.update(self.get_loss(loss, outputs, targets, indices, num_boxes,False, **kwargs))
                ...
                for i, aux_outputs in enumerate(outputs['aux_outputs']):
                    ...
                    for loss in self.losses:
                        ...
                        if loss == 'labels':
                            ...
                            kwargs['log'] = False
                        l_dict = self.get_loss(loss, aux_outputs, targets, indices, num_boxes,enable_aux, **kwargs)
            ...
            if 'enc_outputs' in outputs:
                ...
                for loss in self.losses:
                    ...
                    if loss == 'labels':
                        # Logging is enabled only for the last layer
                        kwargs['log'] = False
                    l_dict = self.get_loss(loss, enc_outputs, bin_targets, indices, num_boxes,enable_aux, **kwargs)
...
def build(args):
    ...
    # 匹配器，用于将预测的boxes与ground truth的boxes进行匹配
